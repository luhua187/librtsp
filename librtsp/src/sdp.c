...
void sdp_dump(struct sdp_payload *sdp)
{
    ...
    printf("v=%d\n", sdp->proto_version);
    printf("o=%s %lld %lld %s %s %s\n", sdp->origin.username,sdp->origin.sess_id, sdp->origin.sess_version, sdp->origin.nettype, sdp->origin.addrtype, sdp->origin.addr);
    printf("s=%s\n", sdp->session_name);
    ...
    if (sdp->zone_adjustments_count) {
        ...
        for (i = 0; i < sdp->zone_adjustments_count; i++)
            printf("%ld %ld%s\n", sdp->zone_adjustments[i].adjust,sdp->zone_adjustments[i].offset, i + 1 < sdp->zone_adjustments_count ? " " : "");
        printf("\n");
    }
